...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiMap, FiCalendar } from 'react-icons/fi';
...
import { getMembros } from 'apiSdk/membros';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['driver', 'passageiro'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<MembroInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/membros` : null),
    () => getMembros({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Membro',
      path: isTenantUser ? `/membros/view/${data?.[0]?.id}` : '/membros',
      entity: 'membro',
